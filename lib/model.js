...
Node.prototype.rpn = function(_start, _end, _level, _lin) {
	...
	for (var i = 0; i < this.children.length; i++) {
		...
		if (start < offset + cl && end > offset) {
			...
			if (child.rpn) {
				child.rpn(s, end - offset, level, lin);
...
Table.prototype.selectedNodes = function(_start, end, _lin) {
	...
	if (start - startCell.offset + startCell.node.length > end) {
		...
		var maxCol = Math.max(startCell.column + (startCell.colSpan || 1) - 1,
			endCell.column + (endCell.colSpan || 1) - 1);
		var maxRow = Math.max(startCell.row + (startCell.rowSpan || 1) - 1,
			endCell.row + (endCell.rowSpan || 1) - 1);
		var crow = 0;
		var ccell = 0;
		for (var i = 0; i < this.children.length; i++) {
