...
function TableRegionNode() {} //NOTE: abstract class
...
function Table      (id, children, attrs) { Node.call(this, id, children, attrs); }
function THead      (id, children, attrs) { Node.call(this, id, children, attrs); }
function TBody      (id, children, attrs) { Node.call(this, id, children, attrs); }
function TFoot      (id, children, attrs) { Node.call(this, id, children, attrs); }
...
TNode.prototype.tagRange = function(ops, start, end, key, value) {
	ops.retain(1);
	var off = 1;
	var current;
	for (var i = 0; i < this.children.length; i++) {
		...
		if (off + cl > start) break;
		if (child instanceof Tag && child.key === key) current = child.value;
		if (child instanceof EndTag && child.key === key) current = undefined;
		off += cl;
		ops.retain(cl);
	}
//TODO
...
TNode.prototype.untagRange = function(ops, start, end, key) {
...
TableRegionNode.prototype.columns = function() {
	for (var i = 0; i < this.children.length; i++) {
		var child = this.children[i];
...
Table.prototype.columns = function() {
	for (var i = 0; i < this.children.length; i++) {
		var child = this.children[i];
		if (child instanceof TableRegionNode)
			return child.columns();
	}
	return 0;
};
...
TableRegionNode.prototype.insertRows = function(ops, row, n, cols) {
...
Table.prototype.insertRows = function(ops, r, n, cols) {
	var _n = n === undefined ? 1 : n;
	var _cols = cols || this.columns();
	if (_cols < 1) return; //no columns (should probably retain self)
	ops.retain(1);
	...
	if (region !== null) {
		for (var i = 0; i < this.children.length; i++) {
			var child = this.children[i];
			var cl = child.length || 1;
			remaining -= cl; //before break so we don't double retain at end
			if (child === region) break;
			ops.retain(cl);
		}
		//retained to region
		region.insertRows(ops, _r, n, _cols);
	}
	ops.retain(remaining);
};
