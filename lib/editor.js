...
function renderChildren(children, _sel, parentId) {
	...
	for (var i = 0; i < children.length; i++) {
		...
		switch (_type) {
			...
			case 'h4':         ret.push(H4({obj:child, selection:subsel, key:keyToId(child.id)})); break;
			case 'h5':         ret.push(H5({obj:child, selection:subsel, key:keyToId(child.id)})); break;
			case 'h6':         ret.push(H6({obj:child, selection:subsel, key:keyToId(child.id)})); break;
			...
			case 'result':     ret.push(Result({obj:child, selection:subsel, key:keyToId(child.id)})); break;
			case 'error':     ret.push(Error({obj:child, selection:subsel, key:keyToId(child.id)})); break;
			case 'html':     ret.push(Html({obj:child, selection:subsel, key:keyToId(child.id)})); break;
			case 'pre':     ret.push(Pre({obj:child, selection:subsel, key:keyToId(child.id)})); break;
			case 'ulli':
				if (cindent > indent)
...
var Row = createClass({
	render: function() {
		...
		for (var i = 0; i < raw_cells.length; i++) {
			...
			if (cell.className) props.className =
...
function offsetFromPoint(textnode, x, y) {
	//console.log("offsetFrom x: " + x + " y:" + y);
	...
	for (;;) {
		...
		//console.log(rects);
		clo = closest(rects);
		//console.log("from: " + from + " middle: " + middle);
		//console.log(JSON.stringify(clo));
		if (clo.y > target.y ||
