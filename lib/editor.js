	...
	render: function() {
		...
		for (var i = 0; i < row.children.length; i++) {
			...
			if (cell instanceof model.Cell) {
				...
				if (children.length === 0) children = ["\u200B"];
				cells.push(DOM[tag || 'td'](props, renderChildren(children)));
...
var Table = createClass({
	render: function() {
		...
		var row;
		var i = 0;
		for (; i < table.headerRows && i < numRows; i++) {
			row = rows[i];
...
var Editor = createClass({
	...
	render: function() {
		...
		this.selection = Selection(); //{colour: 'rgba(114,190,215,0.5);'}); //state actually sets where to render
