...
var KEY_PREFIX = 'clay:';
var reKey = /^clay:/;
var reKeyIndexOffsetCursor = /^clay:([^:]+):(\d+):(\d+)_cursor/;
...
function offsetFromPoint(textnode, x, y) {
	...
	if (len === 0) return 0;
	if (len === 1 && textnode.textContent === "\u2000B") return 0;
	...
	move: function(region, by, amount, forward, extend) {
		...
		switch (by) {
			...
			case 'lines':
				//move to next line
				if (xpos === undefined)
					xpos = this.text_to_window(point).clientX;
				if (forward) {
					//eol
					//move forward 1 char
					//
					...
					//text_to_window() -> y position
					//this.window_to_text({clientX})
				}
				break;
			//case 'wholelines':
