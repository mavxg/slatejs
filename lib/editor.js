...
var VALID_DOC = {
	...
	'table': {
		'row':true,
		_fixup: function(children) { return [wrap('row', childen)]; },
		_level: 2,
	},
	'row': {
		'cell':true,
		_fixup: function(children) { return [wrap('cell', childen)]; },
		_level: 3,

	}
};

//take a document fragment and make it valid
function fixup(list, parent) {
	if (!list) return list;
	if (list.type === 'string') return list; //just a string is valid
	//otherwise we need to check validity and traverse
...
function htmlToDoc(html) {
	...
	elm.innerHTML = html; //
	var res = elemsToDoc(elm.childNodes,{},{});
	if (res.length > 1)
		res = wrap('doc',[wrap('section',res)]);
	else {
		res = res[0]
	}
	return fixup(res);
...
var Editor = createClass({
	...
	pasteText: function(text) {
		...
		//TODO: treat it like markdown and parse stuff
