...
function renderChildren(children, _ret) {
	...
	for (var i = 0; i < children.length; i++) {
		...
		if (child instanceof AttributedString) {
			...
			for (var j = 0; j < chunks.length; j++) {
				...
				if (attr.strong)
					c = DOM.strong({},c);
				if (attr.em)
					c = DOM.em({},c === chunk.str ? c : [c]);
				if (attr.sup)
					c = DOM.sup({},c === chunk.str ? c : [c]);
				if (attr.sub)
					c = DOM.sub({},c === chunk.str ? c : [c]);
				if (attr.href) {
					c = DOM.a({href:attr.href},c === chunk.str ? c : [c]);
				}
				if (typeof c === 'string')
					c = DOM.span({},c === chunk.str ? c : [c]);
				ret.push(c);
			};
			//ret.push(DOM.text(child.str));
			continue;
		}
		...
		switch (_type) {
			case 'doc':   ret.push(Document({obj:child, key:keyToId(child.id)})); break;
			...
			case 'blockquote':      ret.push(Quote({obj:child, key:keyToId(child.id)})); break;
...
function makeClass(tag, className) {
	return createClass({
		render: function() {
			...
			return DOM[tag](props, renderChildren(children));
		}
	});
}
...
var Section = createClass({
	render: function() {
		...
		var children = obj.values.slice(1);
		if (children.length === 0) children = ["\u00A0"];
		var rchildren = renderChildren(children);
...
var Row = createClass({
	render: function() {
		...
		var cells = [];
		var offset = 0;
		var raw_cells = row.values.slice(1);
		...
		for (var i = 0; i < raw_cells.length; i++) {
			...
			var children = cellx.values.slice(1);
			if (children.length === 0) children = [DOM.text("\u200B")];
			else children = renderChildren(children);
