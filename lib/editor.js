		...
		switch (by) {
			case 'characters':
				...
				if (forward) {
					...
					reText.lastIndex = s.length - point + 1;//????
					var i = amount;
					var m;
					while (i > 0 && (m = reText.exec(s))) {
	...
	moveTo: function(region, position, extend) {
		...
		switch(position) {
			...
			case 'hardeol':
				...
				var s = doc.toText();
				var m = s.indexOf('\u009C',point+1);
				if (m > 0) point = m;
				break;
			case 'hardbol':
				//find the first character position in the current breakable
				var s = doc.toText();
				var m = s.lastIndexOf('\u0098',point-2);
				if (m > 0) point = m + 1;
				break;
			case 'eol':
				//v <- text_to_layout(point) //perhaps we have text_to_window
				//y = (v.top + v.bottom) / 2.0
				//clientY = y - this.node.ownerDocument.body.scrollTop;
				//out = window_to_text({clientX:1000000, clientY:clientY})
				break;
			case 'bol':
				//v <- text_to_layout(point) //perhaps we have text_to_window
				//y = (v.top + v.bottom) / 2.0
				//clientY = y - this.node.ownerDocument.body.scrollTop;
				//out = window_to_text({clientX:0, clientY:clientY})
				break;
		}
		if (extend)
			return new Region(point, region.anchor, xpos);
