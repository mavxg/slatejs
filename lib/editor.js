...
var Editor = createClass({
	getInitialState: function() {
		...
		this.focused = false;
		this.poll_id = null;
		return getStoreState(this.props.store);
	},
	didMount: function() {
	...
	readInput: function() {
		...
		if (i < prevInput.length) {
			...
			st -= (prevInput.length - i);
		};

		op.retain(st);

		if (st < en) {
			var lin = this.state.doc.rpn(st, en);
		...
		op.insert(inserted);
		op.end(this.state.doc.length);

		var cur = st + inserted.length;
		var newsel = new _Selection(cur, cur, sel.who);
		this.props.store.apply(op, newsel); //This needs to transform the
	...
	render: function() {
		...
		return DOM.div({className: 'editor',
			...
			this.textarea,
			Preview({doc: this.state.doc})]);
	},
	_onChange: function() {
		this.setState(getStoreState(this.props.store));
	}
});
