		...
		if (child instanceof AttributedString) {
			...
			for (var j = 0; j < chunks.length; j++) {
				...
				for (var k = 0; k < subsubsel.regions.length; k++) {
					...
					var end = r.end();
					if (beg > o) {
						key = keyToId(parentId) + ':' + i + ':' + (coff + 1 + o);
						c.push(DOM.span({id:key},chunk.str.slice(o,beg)));
						o = beg;
					}
					if (r.focus === beg && beg >= 0) {
						key = keyToId(parentId) + ':' + i + ':' + (coff + 1 + beg) + '_cursor';
						c.push(DOM.span({id:key,className:'cursor'},""));
					}
					if (end > o) {
						key = keyToId(parentId) + ':' + i + ':' + (coff + 1 + o);
						c.push(DOM.span({id:key,className:'selected'},chunk.str.slice(o,end)));
					}
					if (r.focus === end && end <= l) {

						key = keyToId(parentId) + ':' + i + ':' + (coff + 1 + end) + '_cursor';
						c.push(DOM.span({id:key,className:'cursor'},""));
					}
					o = end;
				};
				if (o < l || o === 0) {
					key = keyToId(parentId) + ':' + i + ':' + (coff + 1 + o);
					c.push(DOM.span({id:key},chunk.str.slice(o,l)));
				}
				if (attr.strong)
					c = [DOM.strong({},c)];
				if (attr.em)
...
var Editor = createClass({
	...
	window_to_text: function(v) {
		...
		while (elm) {
			...
			var best_distance = [1000000000,1000000000]; //abs(y),abs(x)
			elm = false;
			for (var i = 0; i < nodes.length; i++) {
				var node = nodes[i];
				if (node.nodeType === 3) {
					//text node (assume this is it - one text per span)
					var off = offsetFromPoint(node, x, y);
					return point + off;
	...
	handleMouseDown: function(e) {
		...
		this.anchor = point;
		this.props.store.select(new _Selection([new Region(point)]));
		e.preventDefault();
		e.stopPropagation();
		return false;
	},
	...
	handleBodyMouseUp: function(e) {
		if (this.mouseDown) {
			...
			var point = this.window_to_text(e);
			this.props.store.select(new _Selection([new Region(point,this.anchor || point)]));
		};
	},
