...
Cursor.prototype.toSelection = function(editor, doc) {
	...
	return new _Selection(focus, anchor);
};
...
function rectsForSelection(editor, doc, sel) {
	...
	return doc.selectedNodes(st, en).map(function(n) {
		var node = n.node;
...
var Editor = createClass({
	...
	readInput: function() {
		...
		this.props.store.replaceSelection([inserted],offset);
	...
	handleMouseDown: function(e) {
		this.mouseDown = true; //TODO: might want to only allow selection
		//Mouse down doesn't have the selection set yet
	...
	updateCursors: function() {
		...
		if (this.state.selection.isInverted()) {
			this.top = rects[0].top;
		} else {
			this.top = rects[rects.length - 1].top;
		}
	...
	render: function() {
		...
		return DOM.div({className: 'editor',
			...
			},[this.textarea, this.selection, this.preview]);
	},
	_onChange: function() {
		var store = this.props.store;
		this.setState({
