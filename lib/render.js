...
function renderChildren(children, _sel, parentId, editor, parent) {
	var stack = [];
	var ret = [];
	var wrapped = null;
	var indent = 0;
	...
	for (var i = 0; i < children.length; i++) {
		...
		if (renderer) {
			...
			ret.push(renderer(
				{obj:child, selection:subsel, key:keyToId(child.id), editor:editor, parent:parent}));
			if (renderer.after)
...
function makeClass(tag, className) {
	return createClass({
		...
		render: function() {
			...
			return DOM[tag](props, renderChildren(children, this.props.selection, obj.id, this.props.editor, obj));
		}
	});
}
...
var Section = createClass({
	...
	render: function() {
		...
		if (children.length === 0) children = ["\u00A0"];
		var rchildren = renderChildren(children, this.props.selection, obj.id, this.props.editor, this.props.obj);
...
var Row = createClass({
	...
	render: function() {
		...
		for (var i = 0; i < raw_cells.length; i++) {
			...
			else children = renderChildren(children, subsel, cellx.id, this.props.editor, this.props.row);
