...
function renderChildren(children, _sel, parentId, editor) {
	var stack = [];
	var ret = [];
	var wrapped = null;
	var indent = 0;
	...
	for (var i = 0; i < children.length; i++) {
		...
		if (typeof child.head !== 'function') continue; //skip objects
		...
		var renderer = renderers[_type] || Node;
		if (renderer.wrap && cindent > indent)
			wrap(_type, renderer.wrap,child.id, cindent);
		ret.push(renderer({obj:child, selection:subsel, key:keyToId(child.id), editor:editor}));
		if (renderer.after)
...
function makeClass(tag, className) {
	return createClass({
		...
		render: function() {
			...
			return DOM[tag](props, renderChildren(children, this.props.selection, obj.id, this.props.editor));
		}
	});
}
...
var Section = createClass({
	...
	render: function() {
		...
		if (children.length === 0) children = ["\u00A0"];
		var rchildren = renderChildren(children, this.props.selection, obj.id, this.props.editor);
...
var Row = createClass({
	...
	render: function() {
		...
		for (var i = 0; i < raw_cells.length; i++) {
			...
			else children = renderChildren(children, subsel, cellx.id, this.props.editor);
...
var Preview = createClass({
	render: function() {
		return Document({obj: this.props.doc, selection:this.props.selection, editor:this.props.editor});
	}
});
