...
function commands(editor) {
	...
	function _pasteStr(doc, region, content) {
		return doc.replaceText(region, content.str);
	}

	function _pasteFragment(doc, region, content) {
		console.log('paste to region');
		var as = doc.attributesAt(region.begin());
		var pr = content.prefix(4,content.size-2);
		//TODO: this version will always put in newlines.
	...
	function paste(content) {
		var f = (content.type === 'string') ? _pasteStr : _pasteFragment;
		var ops = bySelection(f, null, content);
		editor.apply(ops);
	}
...
var keymap = [
	...
	{keys:["backspace"], command:"style", args:{style:"p"},
		context:[
			{key:"selection_empty", operator:"equals", operand:true, match_all:true},
			{key:"head", operator:"in", operand:["h1","h2","h3","h4","h5","h6","code","ulli","olli","blockquote","pullquote"], match_all:true},
			{key:"preceding_text", operator:"equals", operand:"", match_all:true},
		]},
