...
var Encrypted = createClass({
	handleEncrypt: function(e) {
		...
		if (pub) {
    		...
    		var armored = openpgp.armor.encode(openpgp.enums.armor.message, 
    			msg.packets.write());
    		console.log(armored);
    		//store {id:keyid ,public:apub, key:armored}
	...
	render: function() {
		...
		//return DOM.div(props, renderChildren(children, this.props.selection, obj.id, editor));
	}
});
...
var Encrypt = createClass({
	render: function() {
		...
		return DOM.div(props, [
			...
			]),DOM.div({className:"content"}, renderChildren(children, this.props.selection, obj.id, editor))]);
	}
});
