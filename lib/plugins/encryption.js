...
EncryptedContext.prototype.importKey = function(ascii, passphrase) {
	...
	if (doDecryption) {
		var dops = this._decryptSnapshot();
		dops = ot._trim(dops);
		if (dops.length > 0) {
...
EncryptedContext.prototype._decryptSnapshot = function(node, offset) {
	offset = offset || 0;
	node = node || this._snapshot;
	if (!node.containsEncrypted()) return [];
	if (node.head().sym === 'encrypted') {
		return this.decryptNode(node,offset);
	} else {
		var dops = [];
		offset += 1;
		for (var i = 0; i < node.values.length; i++) {
			...
			if (child instanceof List) {
				var dop = this._decryptSnapshot(child, offset);
				if (dop.length > 0)
					dops = ot.compose(dops,ot.transform(dop,dops))
				offset += child.size;
			} else if (child instanceof AttributedString) {
				offset += child.size;
			} else {
...
EncryptedContext.prototype.decryptNode = function(node, offset) {
	...
	node.index(2).values.slice(1).forEach(function(o) {
		...
		if (o.iv) {
			//decrypt
			op = cfb.normalDecrypt('aes256', aes, r2s(op), r2s(o.iv));
		}
...
EncryptedContext.prototype.squash = function() {
	...
	for (var i = 0; i < regions.length; i++) {
		...
		var op = this._snapshot.prefix(r.start, r.end).slice(1);
		op.push(ot.operations.retain(1));
		//delete existing ops (slice(1) to take off retain)
		var dops = this._snapshot.prefix(r.ins, r.start,ot.operations.delete).slice(1);
		//insert new op
		dops.push(ot.operations.insert(this.encrypt_op(op,r), 'obj'));
...
function new_encrypt_key(context, editor, pub, id) {
    ...
    var dops = context.decryptNode(nx.node, nx.offset);
    dops = ot._trim(dops);
    context._apply(dops); //server not sent unencrypted ops
