...
List.prototype.replace_results = function(ops, results) {
	console.log(this.head().sym + ' with id ' + this.id + ' ' + this.textContent() + ' it has results ' + this.has_results())
	var exprs = this.qube_expressions();
	console.log('got here sir: ' + this.id + ' it now has results ' + this.has_results())
	if (exprs.length === 0 && !(this.has_results())) {
		console.log('crawling away: ' + this.id)
		ops.push(ot.operations.retain(this.size));
		return;
	}
	console.log('here sir: ' + this.id)
	...
	switch (this.head().sym) {
		case 'result':
			...
			return;
		case 'cell':
			console.log('Checking cell: ' + this.id + ' ' + this.textContent())
			var errors = [];
			var res;
