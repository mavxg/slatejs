		...
		default:
			...
			this.values.forEach(function(x,i) {
				if (typeof x.qube_expressions === 'function') {
					...
					}*/ //what happens if we don't steal results
					rets.push(xs);
					//prev = x;
				}
			});
...
function map_result(res, cell) {
	if (res === undefined || res === null) {
		...
		r.push(res.expr);
		return r;
	} else if (typeof res === 'object' && res.type === 'download') {
		var r = new List();
		r.push(_sym('download'));
		r.push(res);
		return r;
	}
...
var Download = createClass({
	render: function() {
		var obj = this.props.obj;
		var dl = obj.index(1);
		return DOM.pre({},[DOM.a({onClick:this.handleClick,href:'#'},[DOM.text(dl.filename)])]);
	},
	handleClick: function(e) {
		...
		var obj = this.props.obj;
		var dl = obj.index(1);
		var res = editor.run_command('seval',dl.expr);
		console.log(res);
		fs.saveAs(new Blob([res]), dl.filename);
		return false;
	},
});
...
function render_result(res) {
	if (Array.isArray(res) && res.length > 0) {
		...
		return DOM.text((res === undefined ? "undefined" : res).toString());
	}
}
...
var functions = {
	...
	Download: function(filename, expr) {
		return ['Call', ['Symbol', '_Download'], filename, ['JSON', expr]];
	},
	_Download: function(filename, expr) {
		return {
			type:'download',
			filename: filename,
			expr: expr,
...
module.exports = function(editor) {
	...
	var qube = new ql.Qube(functions);
	qube.extend(xl);
	qube.extend(ql.prelude);
	window.qube = qube;
	...
	function seval(sexpr) {
		try {
			...
			return ['Error', "Pending recalculate"];
		}
	}
