...
var Import = createClass({
	...
	render: function() {
		...
		if (imported instanceof List) {
			fa_action = 'refresh';
			url = obj.values[2];
			children = [{}, url]; //need something to skip to get ids right
		} else {
			url = imported;
			imported = undefined;
		...
		children = renderChildren(children, this.props.selection, obj.id, editor);
		children.push(DOM.a({
...
var keymap = [
	{keys:["ctrl+i","i"], command:"insert_import", args:{},
		context:[
	...
	{keys:["enter"], command:"break", args:{type:"line", style:"p"},
		context:[
			{key:"head", operator:"in", operand:["import"], match_all:true}
		]},
