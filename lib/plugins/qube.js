...
module.exports = function(editor) {
	...
	for (var k in ql.prelude) {
		if (ql.prelude.hasOwnProperty(k))
			functions[k] = ql.prelude[k];
	}

	functions.Cache = function(expr, reset_expr) {
		return ['Call', ['Symbol', '_Cache'],
			expr,
			['JSON', expr],
			reset_expr,
			['JSON', reset_expr],
	...
	functions._Cache = function(expr, sexpr, reset, reset_sexpr) {
		return {
			type:'cache',
