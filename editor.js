...
var CONTEXT_OPS = {
	...
	'regex_match': function(value, operand) {
		var r = new RegExp('^' + operand + '$');
		return r.test(value);
	},
	'regex_contains': function(value, operand) {
		var r = new RegExp(operand);
		return r.test(value);
	},
	'not_regex_match': function(value, operand) {
		var r = new RegExp('^' + operand + '$');
		return !(r.test(value));
	},
	'not_regex_contains': function(value, operand) {
		var r = new RegExp(operand);
		return !(r.test(value));
	},
};
...
var R_PRECEDING_TEXT = /^(.+)_preceding_text$/;
var R_FOLLOWING_TEXT = /^(.+)_following_text$/;
var R_SURROUNDING_TEXT = /^(.+)_surrounding_text$/;
...
var Editor = createClass({
	...
	context: function(key, region) {
		...
		switch (key) {
			...
			default:
				var m;
				if ((m = R_PRECEDING_TEXT.exec(key))) {
					n = parentNodeAt(m[1], region.focus)
					if (n) return n.node.toText().slice(0,region.focus - n.point);
				} else if ((m = R_FOLLOWING_TEXT.exec(key))) {
					n = parentNodeAt(m[1], region.focus)
					if (n) return n.node.toText().slice(region.focus - n.point);
				} else if ((m = R_SURROUNDING_TEXT.exec(key))) {
					n = parentNodeAt(m[1], region.focus)
					if (n) return n.node.toText();
				}
				return "";
		}
	},
